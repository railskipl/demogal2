jQuery(document).ready(function() {
  ...
  jQuery("#position_container").sortable({ 
      ...
      update: function(e, ui) {
        ...
        var end_order = jQuery(this).sortable('serialize').replace(/row\[\]\=/g, '').split('&');
        ...
        jQuery.post(BASE_PATH + moved_row_id + '/update_position', {_method:"put", authenticity_token:AUTH_TOKEN, position:end_position}, function(data) {
					json = eval('(' + data + ')');
  ...
  jQuery("table.sortable").tableDnD({
    ...
    onDrop: function(table, row) {
      ...
      jQuery.post(document.location + '/' + row.id + '/update_position', {_method:"put", authenticity_token:AUTH_TOKEN, position:position}, function(data) {
        json = eval('(' + data + ')');
				var icon = '/images/icons/notice.png';
				if (json.title === "Error") icon = '/images/icons/warning.png';
        // inform the user of success / fail via gritter (growl)
	...
	jQuery("table.sortable tr").hover(function() {
